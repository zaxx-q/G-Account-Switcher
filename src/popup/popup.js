...
function findCurrentSiteDomain(url) {
	...
	try {
		...
		const withQueryMatch = matches.filter((d) => {
			...
			return (
				pathOk &&
				parsed.searchParams.get(d.queryMatch.key) === d.queryMatch.value
			);
		});
		...
		const specificMatches = matches
			.filter(
				(d) =>
					d.pathPrefix &&
					!d.queryMatch &&
					parsed.pathname.startsWith(d.pathPrefix),
			)
			.sort(
...
function renderCurrentSite() {
	...
	const defaultIcon = createSiteIcon({
		value: "",
		label: "—",
		caption: currentSettings.globalAccountEnabled
			? `Default (${currentSettings.defaultAccount})`
			: "None",
		isSpecial: true,
...
function createAccountSiteIcon(account, isSelected) {
	...
	btn.addEventListener("click", () =>
		handleCurrentSiteSelection(account.index),
	);
...
function showAddAccountForm() {
	...
	let nextIndex = 0;
	while (
		existingIndices.includes(nextIndex) &&
		nextIndex <= MAX_ACCOUNT_INDEX
	) {
		nextIndex++;
	}
...
function renderSiteSettings(siteSettings, accounts) {
	...
	const existingItems =
		siteSettingListEl.querySelectorAll(".site-setting-item");
	existingItems.forEach((item) => item.remove());
...
function populateSettingSites() {
	...
	const keys = [
		...new Set(
			GOOGLE_DOMAINS.filter((d) => d.type !== "excluded").map((d) =>
				getSiteKey(d),
			),
		),
	];
...
async function handleDetect() {
	...
	try {
		...
		if (response?.success && response.accounts) {
			...
			showStatus(
				"Could not detect accounts. Are you logged in to Google?",
				"error",
			);
		}
	} catch (error) {
...
cancelSiteSettingBtn.addEventListener("click", () =>
	addSiteSettingForm.classList.add("hidden"),
);
...
globalAccountToggle.addEventListener("change", async (e) => {
	...
	await updateSettingsAndWait({
		[STORAGE_KEYS.GLOBAL_ACCOUNT_ENABLED]: enabled,
	});
	renderGlobalSection();
	renderCurrentSite();
	showStatus(
		enabled ? "Global default enabled" : "Global default disabled",
		"info",
	);
