 ...
 */
import { GOOGLE_DOMAINS, getSiteKey, SITE_DISABLED } from "./constants.js";
	...
	for (const domain of GOOGLE_DOMAINS) {
		...
		if (hasSiteSetting) {
			...
			if (domain.type === "path") {
				...
				rules.push(
					...buildQueryMatchRules(
						siteId,
						SITE_QUERYMATCH_PRIORITY,
						domain.host,
						accountNum,
						domain.queryMatch.key,
						domain.queryMatch.value,
						domain.pathPrefix || "",
					),
				);
				siteId += 2;
