...
function findMatchingDomain(url) {
	...
	const withQueryMatch = matches.filter((d) => {
		...
		return (
			pathOk && parsed.searchParams.get(d.queryMatch.key) === d.queryMatch.value
		);
	});
 ...
 */
export async function handleProactiveRedirect(
	tabId,
	url,
	defaultAccount,
	siteSettings,
	...
	if (urlAccount !== null) {
		...
		try {
			...
			if (newUrl && newUrl !== url) {
				...
				console.log(
					`[G-Account Switcher] Proactive (account mismatch): ${url} → ${newUrl}`,
				);
				return true;
			}
		} catch {
 ...
 */
export async function restoreTabSyncStates(
	defaultAccount,
	siteSettings,
	globalAccountEnabled,
) {
	try {
		...
		if (restored > 0) {
			console.log(
				`[G-Account Switcher] Restored sync state for ${restored} tab(s)`,
			);
		}
	} catch (error) {
		console.error(
			"[G-Account Switcher] Failed to restore tab sync states:",
			error,
		);
	}
}
 ...
 */
export async function applyForceSwitch(
	tabId,
	url,
	targetAccount,
	siteSettings,
	...
	try {
		...
		if (changed) {
			...
			console.log(
				`[G-Account Switcher] Force Switch: ${url} → ${parsed.toString()}`,
			);
			return true;
		}
	} catch {}
